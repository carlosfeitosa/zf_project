...
create table basico.tipo_categoria (
	id int identity (1, 1) not null ,
	nome varchar (100) collate latin1_general_ci_ai not null ,
	descricao varchar (2000) collate latin1_general_ci_ai null ,
	rowinfo varchar (2000) collate latin1_general_ci_ai not null 
...
alter table basico_pessoa.assoccl_perfil with nocheck add constraint pk_pessoas_perfis primary key clustered (id) on [primary];

alter table basico.tipo_categoria with nocheck add constraint pk_tipo_categoria primary key clustered (id) on [primary];
...
alter table basico_mensagem.assoccl_assoccl_pessoa_perfil with nocheck add constraint pk_pessoas_perfis_mensagens_categorias primary key clustered (id) on [primary];
...
alter table basico.token with nocheck add constraint pk_token primary key clustered (id) on [primary];

alter table basico.dicionario_expressao with nocheck add constraint pk_dicionario_expressao primary key clustered (id) on [primary];
...
alter table basico_pessoa.assoccl_perfil add
...
create unique index ix_tipo_categoria_nome on basico.tipo_categoria (nome) on [primary];
...
create unique index ix_token_token on basico.token (token) on [primary];

create index ix_dicionario_expressao_constante_textual on basico.dicionario_expressao (constante_textual) on [primary];
...
alter table basico_pessoa.assoccl_perfil add constraint ix_pessoas_perfis unique nonclustered
    (
        id_pessoa,
		id_perfil
	)  on [primary];
	
alter table basico.dicionario_expressao add constraint ix_dicionario_expressao unique nonclustered
	(
		id_categoria,
		constante_textual
	) on [primary];
...
alter table basico.categoriaadd 
	...
	(
		id_tipo_categoria
	) references basico.tipo_categoria (
		id
	);
...
alter table basico_mensagem.assoccl_assoccl_pessoa_perfil add
...
alter table basico.dicionario_expressao add

...
SELECT (SELECT ffe.id
        ...
        AND t.nome = 'FORMULARIO'
        AND c.nome = 'FORMULARIO_ELEMENTO_BUTTON_DIALOG_DOJO'
        AND f.nome = 'FORM_CADASTRAR_USUARIO_VALIDADO') AS id_formulario_formulario_elemento,
       (SELECT f.id
        FROM formulario f
        LEFT JOIN categoria c ON (f.id_categoria = c.id)    
        LEFT JOIN tipo_categoria t ON (c.id_tipo_categoria = t.id)
        ...
        'FORM_FIELD_DOCUMENTOS_IDENTIFICACAO' AS constante_textual_label, 'SYSTEM_STARTUP' AS rowinfo;
